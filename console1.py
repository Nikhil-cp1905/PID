...
try:
    while True:
        ...
        # Interpolate joystick input to motor speed (0 to 127)
        if y_axis < -0.1:  # Forward
            speed = int((y_axis + 1) / 2 * 127)  # Scale from -1 to 1 to 0 to 127
            send_command(64 + speed)  # Move motor 1 forward
            send_command(192 + speed)  # Move motor 2 forward
        elif y_axis > 0.1:  # Backward
            speed = int((-y_axis + 1) / 2 * 127)  # Scale from -1 to 1 to 0 to 127
            send_command(64 - speed)  # Move motor 1 backward
            send_command(192 - speed)  # Move motor 2 backward
        else:  # Stop
